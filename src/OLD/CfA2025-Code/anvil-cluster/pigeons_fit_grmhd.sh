...
INPUT_FILES=("${INPUT_DIR}/hops_3599_MAD_a-0.5_Rh40_i50_LO_preprocessed_snapshot_120_noisefrac0.02_scan73.jls")

# Select the input file for this array task
#INPUT_FILE=${INPUT_FILES[$SLURM_ARRAY_TASK_ID]}
julia_path="/home/x-bnowicki/.julia/juliaup/julia-1.10.10+0.x64.linux.gnu/bin/julia"
...
for INPUT_FILE in "${INPUT_FILES[@]}"; do
  ...
  $julia_path --threads 1 -e "include(\"SgrAfits_grmhd.jl\"); SnapshotModeling.main([\"$INPUT_FILE\"])" > >(tee $output_file) 2> >(tee $err_file >&2)
  ((counter++))
done
