...
function main(args)
    ...
    data_path = joinpath((@__DIR__), "snapshot-modeling/scan185_pigeons/scan185-posterior_chain.jld2")
    mpi_run = joinpath((@__DIR__), "results/all/2025-10-22-16-01-49-0epg7Zrj")
    ...
    if isempty(args)
        snapshot_filepath = joinpath(@__DIR__, "data_comrade",
            "hops_3599_SGRA_LO_netcal_LMTcal_normalized_10s_preprocessed_snapshot_120_noisefrac0.02_scan187.jls")
    else
    ...
    elseif sampler == "pigeons"
        dvis, output_dir, snapshot_ID = load_data(snapshot_filepath, "pigeons_grmhd3599")
    end
    #end

    #output_dir = output_dir * "_grmhd"
    #mkpath(output_dir)
