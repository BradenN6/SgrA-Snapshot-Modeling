...
def make_grid_pdf(image_paths: list[str], output_pdf: str = "grid.pdf",
    ...
    for idx, (ax, path) in enumerate(zip(axes.flat, image_paths)):
        img = imread(path)
        ax.imshow(img)
        ax.set_title(Path(path).stem, fontsize=7, pad=3)
        ax.axis("off")
...
# Pass 16 images explicitly
#python images_to_pdf_grid.py a.png b.png c.png ... p.png

# Use a glob
#python images_to_pdf_grid.py *.png

# Point at a folder (picks first 16 PNGs, sorted by name)
#python images_to_pdf_grid.py --dir ./my_images --output grid.pdf