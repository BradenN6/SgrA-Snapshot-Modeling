...
"""
Dual Cone + Equatorial Plane Emission Model
"""
function skyKrangEq(θ, metadata)
    """
    Use Krang model with Comrade sky function interface.
    θ contains parameters to fit.
    ...
    magfield3 = Krang.SVector(-sin(ι) * cos(η), -sin(ι) * sin(η), cos(ι))
    material3 = Krang.ElectronSynchrotronPowerLawIntensity(magfield2..., vel..., spec, rpeak, p1, p2, subimgs)
    geometry3 = Krang.ConeGeometry(T(π) / 2)
    mesh3 = Krang.Mesh(geometry3, material3)
    
    scene = Krang.Scene((mesh1, mesh2, mesh3))
    kr = KrangModel(incl, met, scene)
    # We include a rotation to rotate the PA of the spin axis
    # We include a stretch to model the M/D (mod) ratio. For Sgr A* you may want to fix this.
