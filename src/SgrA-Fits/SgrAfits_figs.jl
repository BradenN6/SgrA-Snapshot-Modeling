...
function pigeons_sample(post, output_dir, snapshot_ID; mpi=true,
    ...
    if mpi
        setup_mpi(
            ...
            environment_modules = ["gcc", "openmpi"],
...
function pigeons_cluster(post, output_dir, snapshot_ID; mpi=true,
    ...
    setup_mpi(
        submission_system = :slurm,
        environment_modules = ["gcc", "openmpi"],
        #add_to_submission = ["#SBATCH -p wholenode,shared"],
	add_to_submission = ["#SBATCH -p shared"], #, "#SBATCH --nodes=1", "#SBATCH --ntasks-per-node=20"],
        mpiexec = """srun -n \$SLURM_NTASKS""" #--mem-per-cpu 2G""" # --mpi=pmi2
    )
    ...
    println(@capture_out run(`squeue -u x-bnowicki`))
    ...
    if complete == false
        while loop
            if contains((@capture_out run(`squeue -u x-bnowicki`)), String(job_id))
                println("Sampling is ongoing. Sleeping for 1 hour.")
...
function image_reconstruction(post, chain, skym, output_dir, snapshot_ID; im_draws=false, fov=200.0, pix=128)
    ...
    if im_draws
       ...
       for (idx, img) in enumerate(imgs[begin:10:end])
          figim = imageviz_noscale(img)  # grab the grid from skym
...
function imageviz_noscale(
    ...
    pl = _imgviz_noscale!(fig, ax, imguas; scale_length, dkwargs...)
    resize_to_layout!(fig)
    return pl
end

function _imgviz_noscale!(
...
function main(args)
    ...
    mpi_run = joinpath((@__DIR__), "results/all/2025-07-14-13-22-39-4RGuuAVC")
    ...
    elseif sampler == "pigeons"
        dvis, output_dir, snapshot_ID = load_data(snapshot_filepath, "pigeons_figs")
    end
    #end
    ...
    #corner_labels = [L"$a_*$", L"p.a.", L"\theta_o", L"\theta_s", L"p_1", L"p_2",
    #		  L"R_{peak}", L"\chi", L"\iota", L"\beta_v", L"\sigma", L"\eta"]
    ...
    plot_corners(chain, pair_fields, "all", output_dir, snapshot_ID, corner_labels)
