...
function load_data(snapshot_filepath, addendum)
    ...
    output_dir = "snapshot-modeling-grmhdtest3599/" * snapshot_ID * "_" * addendum * "/"
...
function plot_residuals(post, chain, snapshot_ID, output_dir)
    """
    ...
    #axisfields(fig[1, 1], res[1], :uvdist, :res)
    ax = axisfields(fig[1,1], rd[1], :uvdist, :res)
    #ax.ylabelsize = 20
    #ax.xlabelsize = 20
    #axisfields(fig[1,2], rd[2], uvdist, :res)
    #fig = Figure(; resolution=(650, 800))
    #Comrade.plotfields!(fig[1, 1], rd[1], uvdist, :res, axis_kwargs = (; ylabel = "Norm. Res. Vis. Amplitude"))
    #Comrade.plotfields!(fig[2, 1], rd[2], uvdist, :res, axis_kwargs = (; ylabel = "Norm. Res. Vis. Phase"))
...
function main(args)
    ...
    mpi_run = joinpath((@__DIR__), "results/all/2025-11-05-08-37-12-lbSvBVFF")

    if analysis_source == "chain"
        println("Chain Path: $data_path")
    elseif analysis_source == "pt"
        println("PT Path: $mpi_run")
    end
    ...
    if isempty(args)
        snapshot_filepath = joinpath(@__DIR__, "data_comrade",
            "hops_3599_SGRA_LO_netcal_LMTcal_normalized_10s_preprocessed_snapshot_120_noisefrac0.02_scan119.jls")
    else
    ...
    elseif sampler == "pigeons"
        dvis, output_dir, snapshot_ID = load_data(snapshot_filepath, "pigeons_grmhdtest3599")
    end
    #end

    # TODO
    #output_dir = output_dir * "/wide-opening"
    #mkpath(output_dir)
    ...
    priorKrang = (
	    ...
	    mod = DeltaDist(5.03*1.106),  # microas
    ...
    #mimg, simg = image_reconstruction(post, chain, skym, output_dir, snapshot_ID; im_draws=true, fov=200.0, pix=128)
    ...
    plot_corners(chain, pair_fields_trunc, "trunc", output_dir, snapshot_ID)

    plot_residuals(post, chain, snapshot_ID, output_dir)
